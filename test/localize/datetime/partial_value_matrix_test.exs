defmodule Localize.DateTime.PartialValueMatrixTest do
  ...
  format, the standard formats, skeletons, patterns and semantic skeletons.
  ...
  describe "a skeleton or pattern formats the fields it names or names those missing" do
    ...
    test "semantic skeletons" do
      semantic = [
        {Localize.DateTime.SemanticSkeleton.semantic("YMD"), [:year, :month, :day]},
        {Localize.DateTime.SemanticSkeleton.semantic("MD"), [:month, :day]}
      ]

      failures =
        for {skeleton, required} <- semantic,
            locale <- @locales,
            fields <- @subsets,
            module <- modules_for(fields),
            failure <- [check_requested(module, fields, skeleton, locale, required)],
            failure != nil,
            do: failure
  ...
  describe "the halves of a partial datetime" do
    ...
    # medium date-time pattern is "{1}, {0}" and its long "at" pattern
  ...
  defp check_requested(module, fields, format, locale, required \\ nil) do
    required = required || required_fields(format)
