defmodule Localize.DateTime.TimeSkeletonMatrixTest do
  ...
  Three kinds of row follow TR35 where ICU does not, and are checked by the
  tests after the sweep instead:
  ...
  * `J` renders the hour with the digits the locale's pattern shows once its
    ...
    where ICU pads the hour from a 24-hour pattern ("06:00"). In `ko` ICU
    takes the 24-hour `Hms`, "H시 m분 s초", where the preferred `hms` is
    "a h:mm:ss".
  ...
  * `zh-Hant`'s 12-hour formats name the flexible day period ("Bh:mm"); ICU
  ...
  test "J shows the hour without padding it" do
    ...
    assert format("Jms", ~T[18:30:00], "ko") == {:ok, "6:30:00"}
  end
  ...
  defp follows_tr35_instead?(row) do
    shows_midnight_day_period?(row) or padded_capital_j?(row) or ko_24_hour_capital_j?(row) or
  ...
  # ICU writes ko's 24-hour `Hms` with the hour symbol it resolves, so a `J`
  # request shows "h시 m분 s초".
  defp ko_24_hour_capital_j?(%{locale: "ko", skeleton: "J" <> _rest, pattern: pattern}),
    do: String.ends_with?(pattern, "시 m분 s초")

  defp ko_24_hour_capital_j?(_row), do: false
