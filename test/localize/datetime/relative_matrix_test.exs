defmodule Localize.DateTime.RelativeMatrixTest do
  ...
  (`test/support/data/relative_icu_expected.tsv`). ICU 78.3 carries CLDR 48,
  the data Localize ships. Every case goes through `to_string/2` and
  `to_parts/2`: the parts must join to the string, and the number's parts
  must be split as ICU splits them and carry the unit.
  ...
  # fr.xml gives the short and narrow quarter, hour and minute their own
  # "last", "this" and "next" strings, drafted as unconfirmed. The CLDR JSON
  # Localize is built from carries them; ICU's data build leaves unconfirmed
  # data out and renders the long forms ("le trimestre dernier").
  @fr_unconfirmed %{
    {"fr", "short", "quarter", "auto", "-1"} => "le trim. dern.",
    {"fr", "short", "quarter", "auto", "0"} => "ce trim.",
    {"fr", "short", "quarter", "auto", "1"} => "le trim. proch.",
    {"fr", "short", "hour", "auto", "0"} => "cette h",
    {"fr", "short", "minute", "auto", "0"} => "cette min",
    {"fr", "narrow", "quarter", "auto", "-1"} => "trim. dern.",
    {"fr", "narrow", "quarter", "auto", "0"} => "ce trim.",
    {"fr", "narrow", "quarter", "auto", "1"} => "trim. proch.",
    {"fr", "narrow", "hour", "auto", "0"} => "cette h",
    {"fr", "narrow", "minute", "auto", "0"} => "cette min"
  }

  test "relative times match ICU, or CLDR's unconfirmed fr data" do
    mismatches =
      for [locale, style, unit, numeric, value, icu, number, _category] <- rows(@fixture),
          locale not in @root_symbol_locales,
          expected = Map.get(@fr_unconfirmed, {locale, style, unit, numeric, value}, icu),
