defmodule Localize.IntervalClosestMatchTest do
  ...
  # glued two whole dates together, although a width-adjusted item was
  # available.
  #
  # Expectations here were taken from ICU (Node's
  # `Intl.DateTimeFormat.prototype.formatRange`) and then checked against the
  # CLDR 48 data, since ICU 77 ships older CLDR: where the two disagree on a
  # separator or a pattern, CLDR 48 is what we follow.
  ...
  describe "behaviour that must not change" do
    ...
    # CLDR 48 gives vi's `yMd` as "d/M/y – d/M/y" for every difference.
