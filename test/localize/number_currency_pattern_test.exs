defmodule Localize.NumberCurrencyPatternTest do
  ...
  so those symbols come from TR35's pattern character table with CLDR 48's
  `alt="narrow"` symbols. Neither `en` nor `fr` has a narrow Swiss franc
  symbol in CLDR 48, so the standard symbol stands in. TR35's handling of
  invalid patterns gives U+FFFD for four and six currency signs, as ICU
  does. Decimal places set through the API come from ECMA-402's
  `Intl.NumberFormat` in Node 24.
