defmodule Localize.DateTimeTest do
  ...
  describe "to_string/2 with :variant / :standard locales" do
    ...
    # A semantic skeleton with no standard-format equivalent resolves through
    # the available formats, so it reached the same variant.
    test "en-CA semantic month-day skeleton defaults to the standard pattern" do
      skeleton = Localize.DateTime.SemanticSkeleton.semantic("MD", length: :short)

      assert Localize.Date.to_string(~D[2026-05-03], locale: "en-CA", format: skeleton) ==
               {:ok, "05-03"}
    end
  end
