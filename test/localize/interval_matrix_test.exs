defmodule Localize.IntervalMatrixTest do
  ...
  CLDR 48 has changed the locale data since ICU 73's CLDR 43; those cases
  are checked here against the CLDR 48 patterns themselves. Every case goes
  ...
  test "date and time intervals match ICU wherever CLDR 48 agrees with its data" do
    mismatches =
  ...
  describe "data CLDR 48 changed since ICU 73" do
  ...
  describe "the rules TR35 and ICU apply" do
    # fr.xml: `H` is "HH 'h'". Two times in the same hour differ in no field
    # `H` displays, so they format as one.
