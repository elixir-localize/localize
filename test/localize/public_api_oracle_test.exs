defmodule Localize.PublicApiOracleTest do
  ...
  Expected values come from independent sources: CLDR 49's `en` locale
  display names ("Calendar", "Buddhist Calendar"), currency symbols ("$")
  and POSIX responses ("yes:y", "no:n"), and its root ellipsis patterns
  ("… {0}", "{0} … {1}"); ICU4C 78.3's `RuleBasedNumberFormat`
  ("twenty-one", "twenty twenty-four"); ECMA-402 in Node 24 (ICU 77) for
  lists ("a and b") and compound units ("5 liters per kilometer", "5 L/km");
  CLDR's likely subtags ("zh" is "zh-Hans-CN"); TR35's pattern quoting,
  where two single quotes are an apostrophe ("h 'o''clock'"); and the
  digits themselves for Arabic-Indic input.

  """

  use ExUnit.Case, async: true

  test "word ellipses, a quote format that does not exist and responses in the default locale" do
  ...
  test "locale display key and type names in the default locale" do
    assert Localize.Locale.LocaleDisplay.key_name(:ca) == {:ok, "Calendar"}
    assert Localize.Locale.LocaleDisplay.key_name(:ms) == {:ok, "Measurement System"}
    assert Localize.Locale.LocaleDisplay.type_name(:ca, :buddhist) == {:ok, "Buddhist Calendar"}

    assert Localize.Locale.LocaleDisplay.type_name(:co, :phonebook) ==
             {:ok, "Phonebook Sort Order"}

    assert Localize.Locale.LocaleDisplay.type_value_name(true) == {:ok, "On"}
  end
