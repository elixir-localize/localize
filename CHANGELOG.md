...
* `Localize.Date.parse/2`, `Localize.Time.parse/2` and `Localize.DateTime.parse/2` parse a localized string back into a value, so every module that formats one can also read one. They delegate to [calendrical](https://hex.pm/packages/calendrical), resolved at runtime because it depends on Localize; without it they return a `Localize.DependencyRequiredError` naming the package to add.

* `Localize.Options.validate_keys/2` checks a keyword list against the keys a function accepts, returning `Localize.UnknownOptionError` with the nearest known option where there is one.

### Changed

* **Breaking.** `Localize.Number.to_string/2`, `Localize.Date.to_string/2`, `Localize.Time.to_string/2`, `Localize.DateTime.to_string/2` and `Localize.Unit.to_string/2` reject an option key they do not recognise instead of ignoring it. `Localize.Number.to_string(1234.5, currancy: :USD)` returned `{:ok, "1,234.5"}` — a request for currency formatting answered with a bare number — and now returns `{:error, %Localize.UnknownOptionError{}}` suggesting `:currency`.

* **Breaking.** Modules generated by `mix localize.unit.gen_conversions` return a structured exception rather than a tagged tuple. `to_base/2` and `resolve/1` gave `{:error, {:unknown_unit, name}}` and now give `{:error, %YourModule.UnknownUnitError{unit: name}}`, which carries a `message/1`. Regenerate to pick it up; code matching the old tuple needs updating.

### Fixed

* `Localize.DateTime.to_string/2` no longer forwards its own `:date_format`, `:time_format` and `:style` options to the date and time formatters it composes, which do not accept them.

* `Localize.Interval.to_string/3` consumes `:time_format` rather than passing it on to `Localize.Time.to_string/2` after reading it, through both the equal-endpoint and interval paths.

* `Localize.Date.to_string/2` and its siblings accept `:number_system_overrides`, which they read but had never documented as an option.
