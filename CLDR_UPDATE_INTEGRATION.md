...
This is the single authoritative guide for updating Localize to a new CLDR release. It covers the whole cycle: adopting the upstream sources, regenerating locale and supplemental data, updating conformance code against the TR35 changes, verifying, and shipping. A new maintainer should be able to run an update end-to-end from this document alone.
...
At runtime Localize reads only compiled ETF files under `priv/localize/` (plus Unicode Character Database tables under `priv/unicode/`). Those ETFs are generated by the build-time pipeline in `data/` from the raw CLDR sources, read where they sit: the cldr-json release bundle in `$CLDR_PRODUCTION` and the CLDR repository in `$CLDR_REPO`. Nothing is copied into the project; `priv/localize/cldr_json_version` and `priv/localize/cldr_repo_ref` record which release and which ref the committed data was generated from. Only `en.etf` and `und.etf` locale files ship in the hex package; every other locale is downloaded at runtime from the CDN (Cloudflare R2, `https://elixir-localize.com/locales/v<version>/<locale>.etf`) and verified against a SHA-256 hash manifest bundled in the package (`priv/localize/locale_hashes.etf`). Updating CLDR means: refresh sources → regenerate ETFs → adapt code to spec changes → verify → upload to CDN → regenerate the hash manifest from the CDN bytes → release.

## Directory layout

```
$CLDR_PRODUCTION/                  # cldr-json release bundle, outside the project
├── cldr-*-full/main/<locale>/     # Per-locale JSON
├── cldr-core/supplemental/        # Supplemental JSON
└── cldr-rbnf/rbnf/                # RBNF JSON and rule files

$CLDR_REPO/                        # CLDR repository checkout, outside the project
├── common/{supplemental,bcp47,validity,collation,subdivisions}/   # XML
├── common/uca/FractionalUCA.txt   # Fractional UCA table (collation + reorder data)
└── tools/…/Script_Metadata.csv    # Script metadata (script name → subtag)
...
│   ├── cldr_json_version          # cldr-json release generated from (e.g. "49.0.0-BETA1")
│   └── cldr_repo_ref              # CLDR repository tag or commit (e.g. "release-49-beta1")
│
...
* **CLDR production data** — the cldr-json release bundle: the JSON packages cldr-json publishes as `cldr-<version>-json-full.zip` on each release, pre-releases included, unpacked as they are. For a CLDR commit that has no published release, `scripts/build_cldr_production_data` builds the same layout (requires Java/Maven; wraps `cldr-generate-json.sh`). Location: `$CLDR_PRODUCTION`, default `../cldr_production_data`.

* **Only for generating data.** Nothing under `lib/` reads the sources and they are not in the hex package. The test suite does not need them either: in `:dev` and `:test` a locale missing from the cache is generated from the recorded sources when they are present, and downloaded from the CDN otherwise. `mix localize.fetch_sources` fetches the recorded pair onto a machine that lacks them — the release zip into `$CLDR_PRODUCTION`, and a sparse checkout of the paths the pipeline reads into `$CLDR_REPO` — which is how CI generates data.
...
| Task / script | Purpose |
|---------------|---------|
| `mix localize.update_cldr` | **Orchestrates phases 1–3** (record sources → generate supplemental → compile gate → generate locales → test gate), each step in a fresh VM. `--check` runs the preflight and prints the plan without changing anything; `--locales en,fr` trials a subset. |
...
| `mix localize.prepare_sources` | Adopts the sources in `$CLDR_PRODUCTION` and `$CLDR_REPO`: records them in `priv/localize/cldr_json_version` and `priv/localize/cldr_repo_ref`, writes the CLDR version to `priv/localize/version`, fetches the UCD tables via `download_unicode_data`, and copies CLDR's conformance fixtures into `test/support/data/`. A pre-release bundle beside a repository not at that release's tag stops it. |
| `mix localize.fetch_sources` | Fetches the recorded pair onto a machine that lacks it: the cldr-json release zip into `$CLDR_PRODUCTION` and a sparse, blobless checkout of the recorded ref into `$CLDR_REPO`. Never replaces a directory holding anything else. |
| `mix localize.generate_supplemental` | Regenerates supplemental, validity, collation and top-level ETFs under `priv/localize/`. Refuses sources that are not the recorded ones. |
| `mix localize.generate_locales` | Regenerates per-locale ETFs (`priv/localize/locales/`). Positional args scope to specific locales. Refuses sources that are not the recorded ones. |
...
| `mix localize.download_unicode_data` | Fetches the UCD property tables in `priv/unicode/` for the Unicode version `$CLDR_REPO/common/uca/FractionalUCA.txt` names, and is a no-op when they already match. `mix localize.prepare_sources` calls it, so running it directly is only to force a re-check. |
...
Once the upstream sources are in place (step 1 of Phase 1), the record → generate → verify sequence can run as a single orchestrated task:

```bash
mix localize.update_cldr --check    # preflight: verify sources, report versions, print the plan
mix localize.update_cldr            # record sources, generate supplemental (compile gate), generate locales (test gate)
```
...
### Phase 1 — Adopt the sources

```bash
# 1. Put the release's sources in place. cldr-json publishes every release,
#    pre-releases included, as a zip of the packages: unpack it into an empty
#    $CLDR_PRODUCTION, and check the matching tag out in $CLDR_REPO.
curl -fLO https://github.com/unicode-org/cldr-json/releases/download/<version>/cldr-<version>-json-full.zip
unzip -q cldr-<version>-json-full.zip -d $CLDR_PRODUCTION
git -C $CLDR_REPO checkout release-<N>

#    For a CLDR commit with no published release, build the bundle instead.
#    The script fetches $CLDR_REPO, checks out the tag, verifies the JDK, and
#    clears $CLDR_PRODUCTION before building. CLDR 49 needs JDK 21.
JAVA_HOME=$(/usr/libexec/java_home -v 21) \
  CLDR_TAG=release-<N> scripts/build_cldr_production_data

# 2. Adopt them: records the pair in priv/localize/cldr_json_version and
...
#    locale generation raises rather than silently emitting `iso_digits: nil`
#    for every currency. It is vendored in priv/external_sources/.
...
# The UCD property files are handled by prepare_sources above; run this only
# to force a re-check outside a full source refresh.
mix localize.download_unicode_data
```

`mix localize.prepare_sources` fetches the UCD property files for the Unicode version named in the header of `$CLDR_REPO/common/uca/FractionalUCA.txt`, which feeds the collation table, the reorder-group data and the variable-weight boundaries. This used to be a manual step described here, and it was missed for two Unicode releases while the conformance fixtures beside it kept moving; there is nothing to do by hand now beyond reading the versions it reports.

cldr-json can re-spin a release under a new name (`48.2.0-BETA0b`) and publishes packaging-only patches such as 48.2.1 that CLDR never tags, so the release and the ref are recorded rather than derived from each other. For a pre-release, `prepare_sources` still checks that the repository is at the release's tag, ignoring a re-spin's trailing letter.

**Gate:** `priv/localize/cldr_json_version` and `priv/localize/cldr_repo_ref` name the new release and ref, `priv/localize/version` carries the new CLDR version, and `git status` shows the expected fixture deltas under `test/support/data/`. The patch counter auto-resets to 0 at the next generation.
