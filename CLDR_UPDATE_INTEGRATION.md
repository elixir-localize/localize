...
6. **Remaining feature items** from `plans/cldr-49.md`: `localize_emoji` (item 11, needs the annotations packages v2 now emits) and the two 48-retrospective follow-ups (`gmtUnknownFormat` consumer, `FractionalUCA_blanked.txt` ingestion). Semantic skeletons, append items, minimal pairs, min/max significant digits and the `common/testData` audit have all landed.
