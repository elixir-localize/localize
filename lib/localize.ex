defmodule Localize do
  ...
  * `false` otherwise.
  ...
  @doc """
  Returns a list of all known territory atoms.

  Territories include ISO 3166 country codes and CLDR
  region codes (e.g., `:"001"` for the world).

  ### Returns

  * A list of territory atoms.

  ### Examples

      iex> territories = Localize.known_territories()
      iex> :US in territories
      true
  ...
  * `{:error, exception}` if the territory is not known.
  ...
  ### Examples
      ...
      iex> Localize.flag(:US)
      {:ok, "🇺🇸"}

      iex> Localize.flag(:GB)
      {:ok, "🇬🇧"}
  ...
  * `{:error, exception}` if the measurement system is not known.
  ...
  # ── Territory data (Tier 3) ──────────────────────────────────

  @doc """
  Returns a map of territory containment relationships.

  Each key is a territory atom and each value is a list of parent
  territory atoms in order of increasing scope, ending with
  `:"001"` (the world).

  ### Returns

  * A map of `%{territory_atom => [parent_territory_atom, ...]}`.

  ### Examples

      iex> containment = Localize.territory_containment()
      iex> Map.get(containment, :US)
      [:UN, :"001"]

  """
  @spec territory_containment() :: %{atom() => [atom()]}
  def territory_containment do
    Localize.SupplementalData.territory_containment()
  end

  @doc """
  Returns a map of territories to their subdivision codes.

  ### Returns

  * A map of `%{territory_or_subdivision_atom => [subdivision_atom, ...]}`.

  ### Examples

      iex> subdivisions = Localize.known_territory_subdivisions()
      iex> is_list(Map.get(subdivisions, :US))
      true

  """
  @spec known_territory_subdivisions() :: %{atom() => [atom()]}
  def known_territory_subdivisions do
    Localize.SupplementalData.territory_subdivisions()
  end

  @doc """
  Returns a map of subdivision containment relationships.

  Each key is a subdivision atom and each value is a list of
  parent subdivisions and the owning territory.

  ### Returns

  * A map of `%{subdivision_atom => [parent_atom, ...]}`.

  ### Examples

      iex> containment = Localize.known_territory_subdivision_containment()
      iex> is_list(Map.get(containment, :usca))
      true

  """
  @spec known_territory_subdivision_containment() :: %{atom() => [atom()]}
  def known_territory_subdivision_containment do
    Localize.SupplementalData.territory_subdivision_containment()
  end
  ...
  * `{:error, exception}` if the territory is not known.
  ...
  ### Examples
      ...
      iex> Localize.territory_chain(:US)
      {:ok, [:US, :UN, :"001"]}

      iex> Localize.territory_chain(:"001")
      {:ok, [:"001"]}
  ...
  def territory_chain(territory) do
    with {:ok, territory_atom} <- validate_territory(territory) do
      case Map.fetch(territory_containment(), territory_atom) do
        {:ok, chain} ->
  ...
  * `{:error, exception}` if the locale is not valid.
  ...
  ### Examples
      ...
      iex> Localize.default_territory(:en)
      {:ok, :US}

      iex> Localize.default_territory(:ja)
      {:ok, :JP}

  """
  @spec default_territory(atom() | String.t() | Localize.LanguageTag.t()) ::
          {:ok, atom()} | {:error, Exception.t()}
  def default_territory(%Localize.LanguageTag{territory: territory})
