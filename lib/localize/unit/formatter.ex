defmodule Localize.Unit.Formatter do
  ...
  defp format_with_pattern(value, unit_formats, locale, grammatical_case, options) do
    ...
    case select_pattern(unit_formats, grammatical_case, plural, locale, options) do
      {:error, _reason} = error -> error
      {:ok, pattern} -> format_selected_pattern(pattern, value, unit_formats, options)
    end
  end
  ...
  # The engine fallback (`:inflect` option) synthesizes a pattern only
  # when the requested case is absent from the CLDR data; otherwise the
  # normal resolve path applies.
