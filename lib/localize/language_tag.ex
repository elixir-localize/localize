defmodule Localize.LanguageTag do
  ...
  * `{:error, {Localize.NoParentError, message}}` if the locale
  ...
  ### Examples
      ...
      iex> {:ok, parent} = Localize.LanguageTag.parent("en-AU")
      iex> parent.language
      :en
      iex> parent.territory
      :"001"

      iex> {:ok, parent} = Localize.LanguageTag.parent("en")
      iex> parent.language
      :und

  """
  @spec parent(t() | String.t()) :: {:ok, t()} | {:error, {module(), String.t()}}
  def parent(
        %__MODULE__{language: :und, script: nil, territory: nil, language_variants: []} = _tag
      ) do
    {:error, {Localize.NoParentError, "The locale \"und\" has no parent locale"}}
  end

  def parent(%__MODULE__{} = tag) do
    parent_tag =
  ...
  defp lookup_parent_locale(%__MODULE__{} = tag) do
    full_key =
      Locale.locale_id_from(tag.language, tag.script, tag.territory, tag.language_variants)

    case Map.get(@parent_locales, full_key) do
      nil ->
        # Try without script (handles maximized tags like en-Latn-AU → en-AU)
        no_script_key =
          Locale.locale_id_from(tag.language, nil, tag.territory, tag.language_variants)
  ...
  defp find_parent(%__MODULE__{language_variants: [_ | _]} = tag) do
    %{tag | language_variants: [], canonical_locale_id: nil}
  end

  defp find_parent(%__MODULE__{territory: territory} = tag) when not is_nil(territory) do
    %{tag | territory: nil, canonical_locale_id: nil}
  end

  defp find_parent(%__MODULE__{script: script} = tag) when not is_nil(script) do
    %{tag | script: nil, canonical_locale_id: nil}
  end

  defp find_parent(%__MODULE__{} = _tag) do
    {:ok, parsed} = parse("und")
    {:ok, canonical} = canonicalize(parsed)
    canonical
  end
  ...
  defp transfer_extensions(%__MODULE__{} = parent, %__MODULE__{} = child) do
    canonical_id =
      build_canonical_name(%{parent | locale: child.locale, transform: child.transform})

    %{
      parent
      | locale: child.locale,
        transform: child.transform,
        canonical_locale_id: canonical_id
    }
  end
