defmodule Localize.Collation do
  ...
  * `:gt` - if `string_a` sorts after `string_b`.
  ...
  """
  @spec compare(String.t(), String.t(), keyword() | Options.t()) ::
  ...
  def compare(_string_a, _string_b, options)
      when not is_keyword_list(options) and not is_struct(options, Options),
      do: {:error, Localize.Utils.Helpers.invalid_options(options)}

  def compare(string_a, _string_b, _options) when not is_binary(string_a),
    do: {:error, Localize.Utils.Helpers.invalid_value(string_a, "a string")}

  def compare(_string_a, string_b, _options),
    do: {:error, Localize.Utils.Helpers.invalid_value(string_b, "a string")}

  defp compare_strings(string_a, string_b, options) do
  ...
  * `:backend` - `:nif` or `:elixir`. The default is `:elixir`.
  ...
  or `{:error, exception}` if `input` is not a string or a list of code points, or
  the options are not valid.

  A key is only comparable against keys generated by the same Unicode
  version. UCA reassigns primary weights each release, so a character
  inserted ahead of an existing one shifts every weight after it.
  Regenerate persisted keys when the Unicode version changes, and store
  the version alongside them so a mismatch is detectable. `compare/3` is
  unaffected — it re-derives weights on each call.
  ...
  """
  @spec sort_key(String.t() | [non_neg_integer()], keyword() | Options.t()) ::
          binary() | {:error, Exception.t()}
  def sort_key(input, options \\ [])

  def sort_key(input, options) when is_keyword_list(options) do
    with {:ok, options} <- resolve_options(options) do
  ...
  def sort_key(codepoints, %Options{} = options) when is_list(codepoints) do
    with :ok <- validate_codepoints(codepoints) do
      ensure_loaded()
  ...
  def sort_key(input, %Options{}),
    do: {:error, Localize.Utils.Helpers.invalid_value(input, "a string or a list of code points")}

  def sort_key(_input, options), do: {:error, Localize.Utils.Helpers.invalid_options(options)}

  # Code points must be Unicode scalar values for `List.to_string/1` and the
  # weight tables to accept them.
  defp validate_codepoints([codepoint | rest])
       when is_integer(codepoint) and codepoint in 0..0x10FFFF and
              codepoint not in 0xD800..0xDFFF,
       do: validate_codepoints(rest)

  defp validate_codepoints([]), do: :ok

  defp validate_codepoints(codepoints),
    do: {:error, Localize.Utils.Helpers.invalid_value(codepoints, "a list of code points")}
  ...
  * `:backend` - `:nif` or `:elixir`. The default is `:elixir`.
  ...
  """
  @spec sort([String.t()], keyword() | Options.t()) :: [String.t()] | {:error, Exception.t()}
  def sort(strings, options \\ [])

  def sort(strings, options)
      when is_list(strings) and (is_keyword_list(options) or is_struct(options, Options)) do
    with {:ok, options} <- resolve_options(options),
         :ok <- validate_strings(strings) do
      sort_strings(strings, options)
    end
  end

  def sort(_strings, options)
      when not is_keyword_list(options) and not is_struct(options, Options),
      do: {:error, Localize.Utils.Helpers.invalid_options(options)}

  def sort(strings, _options),
    do: {:error, Localize.Utils.Helpers.invalid_value(strings, "a list of strings")}

  defp sort_strings(strings, options) do
    if use_nif?(options) do
  ...
  * `:ok` - tables are loaded and ready.
  ...
  defp resolve_options(options) when is_keyword_list(options) do
    case Keyword.get(options, :locale) do
      ...
      locale ->
        {:error, Localize.InvalidLocaleError.exception(locale_id: locale)}
    end
  end

  defp resolve_options(%Options{} = options), do: {:ok, options}
  ...
  defp options_from_locale_string(locale, extra_options) do
    ...
    case Localize.validate_locale(locale) do
      ...
      _ ->
        ...
        with %Options{} <- Options.new(extra_options) do
          {:ok, Options.from_locale(locale) |> struct(extra_options)}
        end
    end
  end
  ...
  defp options_from_language_tag(%Localize.LanguageTag{} = tag, extra_options) do
    ...
    with %Options{} = resolved_extra <- Options.new(extra_options) do
