...
All entry points return `{:error, exception}` where the exception is a struct carrying the offending value. Unknown features and invalid grammeme values are rejected against the locale's feature model, so an invalid grammeme fails fast rather than silently not inflecting:

```elixir
iex> {:error, error} = Localize.Inflection.inflect("cat", :en, number: "dual")
iex> {error.__struct__, error.value, error.allowed_values}
{Localize.InvalidValueError, "dual", ["plural", "singular"]}

iex> {:error, error} = Localize.Inflection.inflect("cat", :en, sizzle: "plural")
iex> {error.__struct__, error.feature}
{Localize.UnknownFeatureError, "sizzle"}

iex> {:error, error} = Localize.Inflection.pronoun(:en, "garbage", person: :first)
iex> {error.__struct__, error.pronoun}
{Localize.UnknownPronounError, "garbage"}
```
