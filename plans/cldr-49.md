...
**The date-time glue defaulted to the wrong pattern.** *(Corrected: an earlier version of this section claimed cldr-json dropped the `atTime` and `relative` glue patterns. It does not — it emits them as sibling keys, `dateTimeFormats-atTime` and `dateTimeFormats-relative`, and Localize has ingested both all along as `:date_time_at_formats` and `:date_time_relative_formats`. The original check listed the keys *inside* `dateTimeFormats` and never looked for siblings.)*

The real gap was a default. TR35 §Date-Time Combination is explicit: "For a single date with a single time, by default use the `atTime` pattern (if available) to produce an event time … If there is no `atTime` pattern, use the `standard` pattern", and it asks that an API also offer the standard pattern for a *current* time. Localize had the data, the `style: :at` option and the standard fallback — but defaulted to `:default`. `Localize.DateTime.to_string/2` now defaults to `:at`, so `en` at `:long` renders "July 6, 2024 at 2:30:45 PM" rather than "July 6, 2024, 2:30:45 PM"; `:medium` and `:short` are unchanged because CLDR defines the wrapper only for `:full` and `:long`.

That retired two of the eight `@should_be_at_format` exclusions in [test/localize/datetime/conformance_test.exs](../test/localize/datetime/conformance_test.exs) and showed the other six to be misfiled: they are zone symbol substitution, not glue. The skeleton matcher treats `v`, `V`, `O`, `z` and `Z` as interchangeable, so `:MMMMdjmsO` matches a format carrying `z` and renders "GMT" where the requested `O` gives "GMT+0". Each symbol formats correctly on its own; the match loses the request. They now sit with the other timezone exclusions, and that substitution is the next thing to look at in this area.
