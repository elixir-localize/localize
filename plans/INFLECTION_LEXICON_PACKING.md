...
Phase 2 moved packing into the generator, so the published `.etf` carries the packed lexicon and loading is a plain read. It removed the Phase 1 load-time regression outright and made loading far faster than the original map ever was, because no million-entry map is built at any point:

| locale | map (original) | packed at load (Phase 1) | packed in artifact (Phase 2) |
|--------|---------------:|-------------------------:|-----------------------------:|
| `en` | 47 ms | 247 ms | **3 ms** |
| `es` | 266 ms | 1,524 ms | **8 ms** |
| `ru` | 496 ms | 2,369 ms | **8 ms** |
| `de` | 702 ms | 2,903 ms | **17 ms** |
| `ar` | 618 ms | 3,623 ms | **208 ms** |

German loads 41× faster than the original and Russian 62×. Arabic remains the slowest because its ~14,000 patterns still deserialize as ordinary terms.
...
**Phase 2 — pack at generation time. Done.** The generator emits the packed sections directly into each locale's `.etf`, so loading is a plain read and first-touch time drops to milliseconds. Implemented by calling the same packer from `data/inflection_gen/generate.ex`, bumping `@data_revision` to 2, regenerating all 48 locales, and regenerating the hash manifest. The upload workflow derives the CDN path from the sha file plus `@data_revision`, so it publishes to `2333a964e53a-r2/` on the next push to the `inflection` branch; the artifact cache key now also hashes `lib/localize/inflection/lexicon.ex`, since a change there changes the generated bytes.
