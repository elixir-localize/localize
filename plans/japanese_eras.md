...
**Triggered by:** CLDR 49 is dropping era data for every Japanese era before Meiji (era index `< 232`): the start dates in the supplemental data and the era names in every locale. Modern usage relies on era 232–236 (Meiji through Reiwa); ancient eras are an editorial trim, not a correctness fix. We want to keep them because they're load-bearing for any consumer formatting historical Japanese dates — academic publishing, museum cataloguing, genealogical software, calendar conversion utilities — and CLDR has been the canonical machine-readable source for the full set.

**Goal:** Localize owns and maintains all of the data for the eras before Meiji. The dates are the ones we researched; everything else, the era names in every locale included, is CLDR 48's era data. The build merges both into CLDR 49's data, so a later release can correct a date or a name by editing the curated source and regenerating.
...
8. **Update the curated source.** Set the row's `start` in `priv/localize/curated/japanese_eras.json` and run `mix localize.generate_supplemental`; the build does the rest (see *Implementation* below).
...
Localize owns every piece of pre-Meiji era data, and the build merges it into CLDR 49's. Two curated sources feed two generators:

* **Dates** — `priv/localize/curated/japanese_eras.json`, distilled from the research dataset: one proleptic Gregorian start for each of the 237 eras. `Localize.Data.Calendars` generates `priv/localize/supplemental_data/calendars.etf` from CLDR's `calendarData.json` with this list as the Japanese eras.

* **Names** — `priv/localize/curated/japanese_era_names.json`: CLDR 48.2's `eraNames`, `eraAbbr` and `eraNarrow` for eras 0–231. A locale lists only the names that differ from those it inherits (its locale name with the last subtag removed, and finally `und`), which reproduces CLDR 48.2's names for all 766 CLDR 48 locales in 31 entries. `Localize.Data.Locale` merges them over each locale's Japanese era names as it generates the locale, so the curated names hold for every pre-Meiji era.

CLDR's data is kept for the eras it still carries, so a CLDR refresh picks up Meiji onwards while the pre-Meiji data stays ours. Correcting a date or a name is an edit to the curated source followed by `mix localize.generate_supplemental` or `mix localize.generate_locales`.

`priv/localize/supplemental_data/japanese_eras_snapshot_cldr48.etf` freezes CLDR 48.2's 237 era starts, as upstream last published them, for diffing.

### Consumers

Moving the pre-Meiji starts from CLDR's lunisolar values to proleptic Gregorian changes their meaning for a consumer that compensated for the old values. Calendrical did: `Calendrical.Era` converted each pre-Meiji start as a lunisolar date, and `Calendrical.LunarJapanese` counted era years from its year, a lunar-year label. Read that way, the curated starts are converted twice.

## Tasks

* [ ] **Calendrical reads every start as proleptic Gregorian** — `Calendrical.Era` drops its lunisolar conversion (Calendrical `cldr-49`, uncommitted). `LunarJapanese` still needs the lunar year of each proclamation for its era years; see the provenance task.

* [ ] **Provenance in the runtime data** — ship each era's source, status and lunisolar proclamation date, such as `[24, %{start: [898, 5, 20], source: :tsuchihashi_1952, lunisolar: [898, 4, 26], status: :provisional}]`. The validation methodology above defers the lunisolar date to a later release, but `LunarJapanese` counts an era's years from the lunar year of its proclamation: derived from the proleptic Gregorian start that year is wrong for 寛正 (179), proclaimed on 長禄4年12月21日, which the astronomical calendar places in the next lunar year.

* [ ] **Validation tooling** — `mix localize.japanese_eras.audit` prints every row still ❓ or ⚠️ with its citation and exits non-zero while any row is ❓; `mix localize.japanese_eras.diff_cldr` diffs the curated set against upstream CLDR.

* [ ] **Research regeneration** — check in the script that regenerates `plans/japanese_eras_research.json`, as `mix localize.japanese_eras.research`, so CI can hold the dataset in sync.

* [ ] **Japanese calendar guide** — `guides/japanese_calendar.md`: the era machinery, what consumers can expect from the data, and what the provenance fields mean.

* [ ] **Validation pass** — take the Provisional and Disputed rows to Verified with the per-row workflow above. It does not gate the CLDR 49 release; index 167 is the open row.

### Done

* [x] **Era names in the build** — CLDR 48.2's names for eras 0–231 in `priv/localize/curated/japanese_era_names.json`, merged into every locale by `Localize.Data.Locale`. 2026-09-25.

* [x] **Dates in the build** — `Localize.Data.Calendars` generates `calendars.etf` with the curated dates. 2026-09-05.

* [x] **Regression tests** — `test/localize/japanese_eras_test.exs` guards the dates, the provenance flags, the sync with the research dataset and the names in every locale. 2026-09-05, names 2026-09-25.

* [x] **CLDR 48 snapshot** — `priv/localize/supplemental_data/japanese_eras_snapshot_cldr48.etf`.
...
* **Now → CLDR 49 GA (Oct 2026):** the dates and names are in the build. Calendrical reads the curated starts, and the provenance task is decided, before the CLDR 49 release.

* **CLDR 49 GA + 3 months:** the validation pass, its tooling and the guide, shipped in the Localize release after CLDR 49.
